...
export default function OrderScreen() {
  ...
  const { isVatEnabled, vatPercent } = useSettingsStore(
    useShallow((state) => ({ isVatEnabled: state.isVatEnabled, vatPercent: state.vatPercent }))
  );
  ...
  const upsertItemsMutation = useMutation({ 
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['order', tableName] }) 
  });
  
  const updateStatusMutation = useMutation({ 
    mutationFn: ({ orderId, status }: { orderId: number, status: 'paid' | 'cancelled' }) => updateOrderStatus(orderId, status), 
    onSuccess: async () => {
      setCartItems(new Map());
      setPrintedItems(new Map());
      setCurrentOrderId(null);
  ...
  const saveCartToServer = async (): Promise<boolean> => {
    ...
    if (orderId) {
       const items_input = Array.from(cartItems.entries()).map(([id, qty]) => ({ 
  ...
  const handlePrintKitchen = async () => {
    ...
    cartItems.forEach((qty, id) => {
      const diff = qty - (printedItems.get(id) || 0);
      if (diff > 0) itemsToPrint.set(id, diff);
    });

    if (itemsToPrint.size === 0) {
      Alert.alert('Thông báo', 'Hết món mới. In lại phiếu cũ?', [
        { text: 'Không', style: 'cancel' }, 
        { 
          text: 'In lại', 
          onPress: async () => {
             try { await printKitchenBill(tableName, cartItems, menuData || []); } catch(e) {}
          } 
        }
      ]);
      return;
    ...
    try {
      await printKitchenBill(tableName, itemsToPrint, menuData || []);
  ...
  const handlePayment = async () => {
    ...
    printPaymentBill(tableName, cartItems, menuData || [], calculations, () => { 
        if (currentOrderId) updateStatusMutation.mutate({ orderId: currentOrderId, status: 'paid' }); 
    });
  };
  ...
  const filteredMenu = useMemo(() => {
    ...
    return menuData.filter(i => 
      (!search || i.name.toLowerCase().includes(l)) && 
      (selectedCategory === null || i.category_id === selectedCategory)
    );
  ...
  const cartItemsList = useMemo(() => {
    ...
    return Array.from(cartItems.entries())
      .map(([id, q]) => {
        ...
        return i ? { ...i, quantity: q } : null;
      })
      .filter(Boolean) as (MenuItemWithCategory & { quantity: number })[];
  }, [cartItems, menuData]);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.footer}>
        <TouchableOpacity 
          style={[styles.printBtn, { backgroundColor: '#f39c12', flex: 0.8 }]} 
          onPress={handlePreview}
        >
          <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'center'}}>
            <Eye size={20} color="#fff" style={{marginRight: 5}} />
            <Text style={styles.printText}>Xem</Text>
          </View>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  tab: { flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: '#fff' },
  ...
  // Danh mục Compact (Gọn gàng)
  categoryList: { paddingHorizontal: 2 },
  categoryBtn: { 
    backgroundColor: '#fff', 
  ...
  menuName: { fontSize: 15, fontWeight: '600', textAlign: 'center', marginBottom: 4 },
  menuPrice: { color: '#FF6B35', fontWeight: 'bold', fontSize: 14, marginBottom: 8 },
  ...
  qty: { marginHorizontal: 10, fontSize: 15, fontWeight: '600', minWidth: 20, textAlign: 'center' },

  // Cart & Calculation
  cartContainer: { flex: 1 },
  emptyCart: { textAlign: 'center', fontSize: 16, color: '#999', marginTop: 40 },
  cartItem: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', padding: 10, borderRadius: 10, marginBottom: 8, elevation: 1 },
  cartItemInfo: { flex: 1 },
  cartItemName: { fontSize: 15, fontWeight: '600' },
  cartItemPrice: { fontSize: 13, color: '#888' },
  cartQtyRow: { flexDirection: 'row', alignItems: 'center' },
  cartItemTotal: { fontSize: 15, fontWeight: 'bold', color: '#FF6B35', width: 70, textAlign: 'right', marginHorizontal: 8 },
  
  calculationsContainer: { backgroundColor: '#fff', padding: 12, borderRadius: 10, marginTop: 5, elevation: 1 },
  calcRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 },
  calcLabel: { fontSize: 14, color: '#555' },
  calcValue: { fontSize: 14, fontWeight: '600' },
  discountInput: { backgroundColor: '#f0f0f0', borderRadius: 6, padding: 4, fontSize: 14, width: 60, textAlign: 'right' },
  finalTotalRow: { borderTopWidth: 1, borderColor: '#eee', paddingTop: 8, marginTop: 4 },
  finalTotalLabel: { fontSize: 18, fontWeight: 'bold', color: '#FF6B35' },
  finalTotalValue: { fontSize: 18, fontWeight: 'bold', color: '#FF6B35' },
  ...
  printBtn: { backgroundColor: '#3498db', paddingVertical: 12, borderRadius: 10, flex: 1, marginHorizontal: 6, alignItems: 'center', justifyContent: 'center' },
  payBtn: { backgroundColor: '#27ae60', paddingVertical: 12, borderRadius: 10, flex: 1, marginHorizontal: 6, alignItems: 'center', justifyContent: 'center' },
  printText: { color: '#fff', textAlign: 'center', fontWeight: 'bold', fontSize: 15 },
  payText: { color: '#fff', textAlign: 'center', fontWeight: 'bold', fontSize: 15 },
});